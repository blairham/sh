...
func run(ctx context.Context, dialect, bin, buildDir string, timeout time.Duration, jobs int, refetch bool, only string) int {
	...
	// Before anything is fetched or run, because every figure this instrument
	// prints is inflated when the two shells' base names differ and nothing in
	// the report says so. See suite.CheckBaseNames, which is normalize's
	// precondition written down.
	switch warning, err := suite.CheckBaseNames(s, bin, reference); {
	case err != nil:
		fmt.Fprintf(os.Stderr, "suitecheck: %v\n\n", err)
		fmt.Fprintln(os.Stderr, "  The comparison strips each shell's full path and never its base name, so a")
		fmt.Fprintln(os.Stderr, "  line where a shell names itself by base name is compared as written. With")
		fmt.Fprintln(os.Stderr, "  different base names every one of those lines differs, and nothing in the")
		fmt.Fprintln(os.Stderr, "  report below would have said why.")
		fmt.Fprintf(os.Stderr, "\n  Build or copy the shell under the name %q — the path may be anywhere,\n"+
			"  only the base name is compared.\n", filepath.Base(reference))
		return 2
	case warning != "":
		fmt.Printf("suitecheck: %s\n\n", warning)
	}
...
func printDiffering(o out, rep suite.Report) {
	...
	// The three floors, each printed when it has something to say. The prose
	// one is gated on the column having been asked — a column with no
	// [suite.Suite.SelfDoc] has a Prose of zero because the question was
	// never put, which is a different thing from an answer of none. The
	// other two are computed from the two runs alone, so they print for
	// every column; they used to sit behind the same gate and were invisible
	// in a column that had no self-documentation to ask for.
	if rep.ProseAsked() {
