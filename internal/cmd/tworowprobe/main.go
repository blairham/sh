...
// two-row prompt and reports what reached the screen.
//
...
// binary, which is the thing a person actually runs — components can each
// match the shell they imitate while the composition of them does not.
//
// It drives `bash` and not `zsh`: the prompt is handed over in the
// environment, and bash takes PS1 from there for an interactive shell where
...
func main() {
	...
	paste := flag.Bool("paste", false, "send a bracketed paste instead of recalling history")
	flag.Parse()
	if *bin == "" {
	...
	// Zero is the answer #2469 landed and the answer real bash gives; a
	// ladder of upper rows is what the bug looked like.
	fmt.Printf("the recall interaction added %d copies of the upper row (want 0)\n", after-before)
