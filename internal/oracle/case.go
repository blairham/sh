...
echo unreachable`,
	...
	},
	{
		...
		Unfinished: true,
		Snippet:    "cat <<X\nbody",
		Why:        "the row above it with the final newline taken away, which is the only way to write a here-document body that does not end in one — and it splits the panel on the body itself rather than on a remark: bash supplies the missing newline and writes `body` and a newline, where bash 3.2, dash, ksh93 and zsh hand over the four bytes they read. bash 5.3 still remarks on the delimiter that never came and bash 3.2 still does not. It is the shape that found #962: the printer wrote the delimiter onto that unfinished line and the body became `bodyEND`",
	},
	{
