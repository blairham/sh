...
// `[.x.]` is a collating element and `[=x=]` an equivalence class, in every
// column but one — see Semantics.CollatingSymbols. Which preset is which is
...
// nobody measured.
func collatingSem(a Answer, unknown UnknownClassPolicy) Semantics {
	s := permissive()
	s.CollatingSymbols = a
	s.UnknownCharacterClass = unknown
	return s
}

// collates answers whether pattern matches subject with the axis at a.
func collates(t *testing.T, a Answer, unknown UnknownClassPolicy, pattern, subject string) bool {
	t.Helper()
...
func TestACollatingElementIsOneMemberOfTheBracket(t *testing.T) {
	for _, tc := range []struct {
		...
		if got := collates(t, Yes, UnknownClassIsInert, tc.pattern, tc.subject); got != tc.on {
			t.Errorf("on: %s vs %q = %v, want %v", tc.pattern, tc.subject, got, tc.on)
		}
		if got := collates(t, No, UnknownClassIsInert, tc.pattern, tc.subject); got != tc.off {
...
func TestABodyThatIsNotAnElementIsTheUnknownNameQuestion(t *testing.T) {
	for _, tc := range []struct {
		...
		for _, c := range []struct {
			...
			if got := collates(t, Yes, c.policy, tc.pattern, tc.subject); got != c.want {
...
func TestAnUnclosedCollatingDelimiterIsOrdinaryWhereTheReadingIsInert(t *testing.T) {
	for _, tc := range []struct {
		...
		for _, c := range []struct {
			...
			if got := collates(t, Yes, c.policy, "[x[.a]y]", tc.subject); got != c.want {
