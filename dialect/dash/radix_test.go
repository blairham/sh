...
//
// Measured 2026-09-22 on macOS 15 against dash with the host's own
...
//
// The C locale's answer to all three, which is this shell's answer to every
// locale question: it never consults `LC_NUMERIC` at all, so the **comma** is
// the operand it refuses here where the other three refuse the point.
//
...
func TestTheRadixCharacterFollowsTheLocale(t *testing.T) {
	for _, c := range []struct {
		...
		{name: "a comma operand", src: `printf '%.2f' 1,5`, want: "1.00", refuses: "1,5"},
	} {
