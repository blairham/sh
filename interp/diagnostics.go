...
type Diagnostics struct {
	...
	// HistoryNoPreviousSubstitution is a `:&`, or a substitution with an
	// empty left side, before anything has been substituted, and `%[1]s` is
	// the modifier as written. Measured 2026-09-16: bash from a script and
	// ksh93 at a prompt say `:g&: no previous substitution`, and zsh at a
	// prompt says `no previous substitution`.
	HistoryNoPreviousSubstitution string
	...
	// reach it. Nil falls back to ReadonlyRefusalNamesBuiltin, which is what
	// every dialect but one wants.
	//
	// A set of its own because one shell answers the two shapes differently
	// through the identical word. Measured 2026-09-07 and 2026-09-20:
	//
	//	ksh93  typeset x=2                → <script>: line 2: x: is read only
	//	       typeset +r x               → <script>[2]: typeset: x: is read only
	//	       readonly c; typeset -i c   → <script>[2]: typeset: c: is read only
	//
	// One word, two sentences and two locations, so the builtin's name
	// cannot decide it alone. What that shell parts on is whether the
	// refusal is about a **value** or about an **attribute**: the attribute
	// shapes take what `set -A` takes — the builtin's own location with the
	// builtin named — and an assignment through the same word takes the
	// plain line form. bash names the builtin for all of them and needs no
	// second answer.
	ReadonlyAttributeRefusalNamesBuiltin map[string]bool
