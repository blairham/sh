...
func TestAMissingBuiltinIsNotLookedForOnPath(t *testing.T) {
	for _, name := range []string{"umask", "ulimit", "alias", "unalias", "hash", "type", "jobs", "fg", "bg"} {
...
func TestRegisteringOneStopsItBeingReserved(t *testing.T) {
	...
	setup := func(r *Runner) {
		r.Register("umask", func(r *Runner, _ context.Context, _ []string) int {
			_, _ = r.Stdout.Write([]byte("the builtin ran\n"))
			return 0
		})
	}
	out, st, err := reservedRun(t, dir, "umask 077", setup)
	if err != nil {
...
func TestAFunctionStillShadowsAReservedName(t *testing.T) {
	dir := t.TempDir()
	shadow(t, dir, "umask", "echo ran-the-external")
	out, st, err := reservedRun(t, dir, "umask() { echo the-function; }\numask 077", nil)
	if err != nil {
