...
//
// Measured 2026-09-22 on macOS 15 against bash 5.3.20 with the host's own
...
//
// bash 3.2.57 answers all three rows the same way, so this is a reading and not
// a release. The refused operand still writes its leading number, which is
// measured: `1,00` rather than `0,00`.
//
