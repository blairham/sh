...
func TestAnswersTheInterpAxisTestsRelyOn(t *testing.T) {
	...
	// no parameter for the order an expansion comes back in.
	...
	if got, want := s.ReadTrailingEscapedSeparator,
		interp.ReadTrailingEscapedSeparatorKept; got != want {
	...
	if got, want := s.LastBackgroundPid, interp.LastBackgroundPidUnset; got != want {
