...
// A colon closes a tilde prefix in an ordinary word **only where the whole word is written plainly** here. See
...
func TestAColonAndAnOrdinaryWordsTildePrefix(t *testing.T) {
	for _, tc := range []struct{ name, src, want string }{
		...
		{"two colons", `printf "[%s]" ~::x`, "[/h::x]"},
		// The quote rows, which is where this column parts from ksh93: any
		// quoting past the tilde turns the reading off.
		{"a quote past the colon", `printf "[%s]" ~:x"y"`, "[~:xy]"},
		{"a single quote past it", `printf "[%s]" ~:x'y'`, "[~:xy]"},
		{"a backslash past it", `printf "[%s]" ~:x\y`, "[~:xy]"},
		{"a quoted tail on its own", `printf "[%s]" ~:"x"`, "[~:x]"},
		// And the control that says it is the colon reading rather than the
		// quote: with a slash first the prefix has already ended, and the
		// quote behind it changes nothing.
		{"a slash first, then a quote", `printf "[%s]" ~/m:"x"`, "[/h/m:x]"},
