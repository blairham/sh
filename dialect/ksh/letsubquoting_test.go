...
// A `let` operand's subscript is not a quoting context here, where the same
// brackets inside `(( … ))` are one — so this is the neighbor of
// ArithSubscriptQuotationMustClose and not a second spelling of it.
//
...
func TestALetOperandsSubscriptIsTakenAsWrittenHere(t *testing.T) {
...
func TestTheLetOperandsSubscriptQuotingIsAnAxis(t *testing.T) {
	if got := ksh.Semantics().LetOperandSubscriptIsAQuotingContext; got != interp.No {
