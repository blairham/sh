...
func (r *Runner) assignOperands(ctx context.Context, c *syntax.SimpleCmd) {
	if r.operandWasRejoined {
		...
		//
		// In front of the held-trace bookkeeping below rather than behind it:
		// there is nothing to store, so there is no trace to put in front of
		// anything, and the function should leave that state exactly as it
		// found it. The `defer` would restore it either way — this only says
		// so without the round trip.
		return
	}
