...
//
// Measured 2026-09-22 on macOS 15 against ksh93u+ (AJM 93u+ 2012-08-01) with the host's own
...
//
// bash's value, measured separately rather than inherited: this shell words the
// refusal `arithmetic syntax error` where bash says `invalid number`, which is
// the diagnostic and not the axis. The leading number is written after it, and
// `.5`, `1.` and `1.5x` all draw the same shape — the last with no locale
// involved at all.
//
...
func TestTheRadixCharacterFollowsTheLocale(t *testing.T) {
	for _, c := range []struct {
		...
		{name: "a point operand", src: `printf '%.2f' 1.5`, want: "1,00", refuses: "1.5"},
