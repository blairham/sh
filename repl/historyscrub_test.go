...
func TestACredentialIsNotWrittenToTheHistoryFile(t *testing.T) {
	...
	if err := h.save(t.Context(), nil, added, nil, false); err != nil {
		t.Fatal(err)
	}
...
func TestNothingButCredentialsWritesNoFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "hist")
	if err := (historyFile{path: path, size: 100, file: 100}).save(t.Context(), nil, []string{"export GH_TOKEN=" + fakeToken}, nil, false); err != nil {
		t.Fatal(err)
	}
...
func TestTheNoticeSaysWhyAndWhich(t *testing.T) {
	...
	// the session.
	path := filepath.Join(t.TempDir(), "hist")
	if err := (historyFile{path: path, size: 100, file: 100}).save(t.Context(), nil, e.history, nil, false); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(path); err == nil {
