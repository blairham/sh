...
// A colon closes a tilde prefix in an ordinary word **whatever else the word holds** here. See
...
func TestAColonAndAnOrdinaryWordsTildePrefix(t *testing.T) {
	for _, tc := range []struct{ name, src, want string }{
		{"a colon closes it", `printf "[%s]" ~:x`, "[/h:x]"},
		{"a colon then a path", `printf "[%s]" ~:x/y`, "[/h:x/y]"},
		{"a colon ending the word", `printf "[%s]" ~:`, "[/h:]"},
		{"a colon then a tilde", `printf "[%s]" ~:~`, "[/h:~]"},
		// Where bash declines, this column does not: quoting past the tilde
		// changes nothing here.
		{"a quote past the colon", `printf "[%s]" ~:x"y"`, "[/h:xy]"},
		{"a quoted tail on its own", `printf "[%s]" ~:"x"`, "[/h:x]"},
		{"a backslash past it", `printf "[%s]" ~:x\y`, "[/h:xy]"},
