...
// on macOS arm64:
//
//	kill -99 $$    kill: Illegal option -9    status 2
//	kill -32 $$    kill: Illegal option -3    status 2
//	kill -s 99 $$  kill: invalid signal number or name: 99    status 2
//
// The second row is what proves it is the first character and not the value:
// a shell reading the whole word would name `-32`.
func TestADashWordOfDigitsIsOptionLetters(t *testing.T) {
	...
	for _, c := range []struct{ src, want string }{
		{`kill -99 $$`, "Illegal option -9"},
		{`kill -32 $$`, "Illegal option -3"},
		{`kill -s 99 $$`, "invalid signal number or name: 99"},
	} {
